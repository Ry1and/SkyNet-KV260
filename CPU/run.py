...
def main():
    ...
    anchor_w = anchor_w.repeat(batch_size, 1).repeat(1, 1, h * w).view(batch_size * num_anchors * h * w).to(device)
    anchor_h = anchor_h.repeat(batch_size, 1).repeat(1, 1, h * w).view(batch_size * num_anchors * h * w).to(device)
    sz_hw = h * w
    ...
    for batch_idx, data in enumerate(test_loader):
        ...
        if batch != batch_size:
            ...
            grid_x = torch.linspace(0, w - 1, w).repeat(h, 1).repeat(batch * num_anchors, 1, 1).view(
                batch * num_anchors * h * w).to(device)
